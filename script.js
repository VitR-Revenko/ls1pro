...
$paginationButtons.forEach(($button) => {
	// console.log($button.getAttribute('data-btn'))
	if ($button.getAttribute('data-btn') === 'prev') {
		console.log('true')
		$paginationButtonPrev.addEventListener('click', () => {
			if ($paginationButtonPrev.classList.contains(disabledClass)) {
				return;
	...
	if ($button.getAttribute('data-btn') === 'next') {
		console.log('true')
		$paginationButtonNext.addEventListener('click', () => {
			if ($paginationButtonNext.classList.contains(disabledClass)) {
				return;
...
const $formDataItems = Array.from(document.getElementsByClassName("js-form-data"));
...
$form.addEventListener("submit", (e) => {
  ...
  for (let $item of $formDataItems) {
    ...
    const $currentInput = $inputs.find(($input) => $input.getAttribute("name") === dataAttr);
